...
sudo proxychains debootstrap $DEBOOTSTRAP_PARAMS
...
if [ $PERF = "true" ]; then
    ...
    sudo proxychains chroot $DIR /bin/bash -c "apt-get update; apt-get install -y flex bison python-dev libelf-dev libunwind8-dev libaudit-dev libslang2-dev libperl-dev binutils-dev liblzma-dev libnuma-dev"
